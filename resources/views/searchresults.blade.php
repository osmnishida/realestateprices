        ...
        @switch($resp["Type"])
          ...
          @endphp
            ...
            <p>取引の種類:{{ $resp["Type"] }}　　　地区:{{ $resp["Region"] }}　　　都道府県名:{{ $resp["Prefecture"] }}　　　
              ...
              取引価格（総額）：{{ $resp["TradePrice"] }}　　　坪単価:{{ $resp["PricePerUnit"] }}　　　
            ...
            @php
              $modefiedResp = [];
              $modifiedResp[] = $resp;
              // var_dump($modifiedResp);
            @endphp
            @break
        @endswitch
      @endforeach
      @php
        $jsonModifiedResp = json_encode($modifiedResp);
        // var_dump($jsonModifiedResp);
      @endphp
        <form method="post" action="landpost">
          @csrf
          <input type="hidden" name="modifieldresp" value=@php echo $jsonModifiedResp @endphp>
